...
document.addEventListener("DOMContentLoaded", function(event) {
    window.onload = function() {
        ...
        window.requestAnimationFrame(function() {
            cursor();
            new _grid.Grid(document.querySelector('.base'));
        });
    };
});

},{"./grid":"5bMWt","./cursor":"3v1v0"}],"5bMWt":[function(require,module,exports) {
...
class Grid {
    ...
    constructor(DOM_el){
        ...
        _scrollTrigger.ScrollTrigger.saveStyles([
            this.DOM.columns,
            this.DOM.left,
            this.DOM.right,
            this.DOM.center
        ...
        _scrollTrigger.ScrollTrigger.matchMedia({
            "(min-width: 800px)": ()=>{
                ...
                this.timeLine.from(this.DOM.left, {
                    duration: 1,
                    rotationX: -0.3,
                    y: ()=>-1 * (this.DOM.right.offsetHeight - winsize.height)
                }, 0).to(this.DOM.center, {
                    duration: 1,
                    rotationX: 0.3,
                    y: ()=>-1 * (this.DOM.center.offsetHeight - winsize.height)
    ...
    initHeading() {
        this.enableFilter = ()=>{
            ...
            _gsap.gsap.set(this.DOM.cursor, {
                filter: "url(#goo)"
            });
        };
        this.transitionEnd = ()=>{
            ...
            winsize = {
                width: window.innerWidth,
                height: Math.max(window.innerHeight || 0, document.body.clientHeight || 0)
            };
            _gsap.gsap.to(this.DOM.middle, {
                duration: .5,
                opacity: 0
            });
            this.tweenEnd = _gsap.gsap.to([
                this.DOM.up,
                this.DOM.down
            ], {
                duration: 1,
                y: _gsap.gsap.utils.wrap([
                    -(winsize.height / 2 - this.titleSize / 2 - 12),
                    winsize.height / 2 - this.titleSize / 2 + 18
                ])
            });
            this.tweenEnd.play();
            _gsap.gsap.to(this.DOM.cursor.firstElementChild, {
                duration: 1.1,
                scale: 1,
            ...
            _gsap.gsap.to([
                ...
                duration: 1,
                yPercent: _gsap.gsap.utils.wrap([
                    0,
                    0
                ]),
                ease: "Sine.out"
            });
            _gsap.gsap.to(this.DOM.center, {
                duration: 1,
                yPercent: 0,
                ease: "Sine.out"
            });
            // bottom title
            //tood : need to look
            this.ss = _gsap.gsap.to(this.DOM.bottom, {
                autoAlpha: 1,
                y: '+=10',
                duration: .3,
                ease: 'Power1.in',
            ...
            _scrollTrigger.ScrollTrigger.create({
                ...
                onUpdate: ({ progress , direction , isActive  })=>{
                    if (direction == -1) this.ss.reverse();
                    if (direction == 1) this.ss.play();
                    else if (direction == 1 && isActive == true) this.ss.play();
                }
            });
        };
        _gsap.gsap.set([
        ...
        _gsap.gsap.set(this.DOM.center, {
            yPercent: 50
        });
        _gsap.gsap.set([
            this.DOM.up,
        ...
        _gsap.gsap.set(this.DOM.cursor.firstElementChild, {
            scale: 100,
            x: winsize.height / 2,
            y: winsize.height / 2
        });
        this.tweenStart = _gsap.gsap.from([
            this.DOM.up,
            this.DOM.down
        ], {
            duration: 1.6,
...
/* https://codepen.io/mendieta/pen/WgvENJ -> Ricardo Mendieta @mendieta */ var _gsap = require("gsap");
...
class Dot {
    ...
    draw(delta) {
        if (!idle || this.index <= sineDots) _gsap.gsap.set(this.element, {
            x: this.x,
            y: this.y
        });
        else {
...
const positionCursor = (delta)=>{
    ...
    dots.forEach((dot, index, dots1)=>{
        let nextDot = dots1[index + 1] || dots1[0];
        dot.x = x;
        dot.y = y;
        dot.draw(delta);
...
},{"gsap":"fPSuC"}]},["ljsGX","ebWYT"], "ebWYT", "parcelRequire36ed")
